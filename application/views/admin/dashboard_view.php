...
<body>
            ...
            <ul class="nav justify-content-center">
                <li class="nav-item active">
                    <div style="display: flex;justify-content: center;margin: 15px;">
                        <a class="nav-link" href="index.php" style="color:#34827a">Home</a>
                    </div>
                </li>
                <li class="nav-item">
                    <div style="display: flex;justify-content: center;margin: 15px;">
        ...
        <div class="container-fluid" style="display: flex;justify-content:center">
            <div class="allblogs">
                <?php 
                    ...
                    $result = $this->db->query("SELECT * FROM `blogs` where status=1 order by created_on DESC ");
                    $imagepath = base_url();
                    if($result->num_rows()>0)
                    {
                        ...
                        for($i = 0;$i<$totalBlogs;$i++)
                        {
                            ...
                            echo '<button class="btn delete" style="background-color:green;color:white" onclick=unpublishBlog('.$blogsList[$i]["blogid"].')>Unpublish</button>';
                            echo "</div>";
                            echo'</div>';
                            echo'<div style="display:flex;justify-content:space-evenly">';
                            echo'<p>Posted on ';
    ...
    <script>
        ...
        {
            Swal.fire({
                title:"Do you want to unpublish the blog",
                text:"You can again publish the blog from Unpublished blogs section",
                icon:"info",
                showCancelButton:true,
                cancelButtonText:"Cancel",
                ...
                {
                    var xhttp = new XMLHttpRequest();
                    xhttp.open("POST","<?= base_url()?>index.php/admin/dashboard/unpubBlog/"+id);
                    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                    xhttp.send("id="+id);
                    xhttp.onreadystatechange=function(){
                        if(this.readyState==4 && this.status==200)
                        {
                            ...
                            if(data=="success")
                            {
                                Swal.fire("Unpublished successfully","","success").then((result)=>{
                                    if(result.isConfirmed){
                                        location.reload();
                                    }
                                    else{
